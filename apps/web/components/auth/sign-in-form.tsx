...
export interface SignInFormProps {
  /** Heading above the form. Defaults to "Sign in to Kestrel". */
  heading?: string;
  /** Supporting line under the heading. */
  subheading?: string;
  /**
   * Accessible foreground colour for the submit button (validated hex from
   * firm branding). When set, applied as an inline style so light brand
   * colours keep readable button text.
   */
  accentForeground?: string;
}

function SignInFormInner({
  heading = "Sign in to Kestrel",
  ...
  const submitStyle: CSSProperties | undefined = accentForeground
  ...
  return (
    <div>
      ...
      <form action={handleSubmit} className="mt-6 space-y-4">
        ...
        <Button type="submit" className="w-full" style={submitStyle} disabled={loading}>
...
/**
 * The Kestrel sign-in form (OAuth + email/password), extracted so both the
 * standard `/sign-in` page and the firm-branded `/f/[slug]/sign-in` page share
 * identical behaviour: error banners, password reveal, and post-sign-in
